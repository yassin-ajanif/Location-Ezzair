...
public partial class ReservationEditView : UserControl
{
    ...
    private void OnFactureChipTapped(object? sender, TappedEventArgs e)
    {
        if (DataContext is not ViewModels.ReservationEditViewModel vm) return;
        e.Handled = true;
        vm.ToFactureCommand.Execute(null);
    }

    private void OnSoftReservationChipTapped(object? sender, TappedEventArgs e)
    {
        if (DataContext is not ViewModels.ReservationEditViewModel vm) return;
        e.Handled = true;
        vm.OpenSoftReservationCommand.Execute(null);
    }
